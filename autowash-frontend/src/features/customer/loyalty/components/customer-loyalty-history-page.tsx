...
import { formatLoyaltyPoints, formatLoyaltyTransactionType } from "@/features/customer/loyalty/lib/customer-loyalty";
import { useCustomerLoyaltyTransactions } from "@/features/customer/loyalty/hooks/use-customer-loyalty";
      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-6">
        <div className="flex items-center justify-between">
          <Link href="/customer/loyalty" className="text-lg font-semibold text-slate-900 hover:text-slate-700">
            {translate("Quay lại", "Back", language)}
          </Link>
        </div>
        ...
        <Card className="rounded-3xl border border-slate-200 bg-white shadow-sm">
          <CardHeader className="border-b border-slate-200 pb-6">
            <div className="space-y-4">
              <div>
                <CardTitle>{translate("Lịch sử giao dịch điểm", "Point transaction history", language)}</CardTitle>
              </div>
              <Button
                type="button"
                variant="outline"
          ...
          <CardContent className="space-y-4 p-6">
            {transactionsQuery.isPending ? (
              ...
              <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6 text-sm text-slate-600">
                {translate("Giao dịch điểm sẽ xuất hiện ở đây sau khi hoàn thành phiên rửa xe.", "Point entries appear here after a wash session is completed.", language)}
              </div>
            ) : (
