...
export function CustomerBookingDetailPage({ bookingId }: { bookingId: string }) {
  ...
  const needsOtpVerification =
    bookingQuery.data?.status === "PENDING" && bookingQuery.data?.confirmationStatus === "PENDING";
  const bookingResolved = Boolean(bookingQuery.data && bookingQuery.data.status !== "PENDING" && !needsOtpVerification);
          ...
          {needsOtpVerification ? (
            ...
            <Card className="border-emerald-200 bg-emerald-50 shadow-md">
              <CardHeader>
                ...
                <CardDescription>
                  The backend has moved this booking out of OTP confirmation. Refresh to sync the latest status.
                </CardDescription>
              </CardHeader>
              <CardContent className="flex flex-wrap gap-3">
