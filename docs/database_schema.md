...
Cấu trúc dưới đây phản ánh chính xác bản thiết kế database mới nhất hiện đang chạy (tham chiếu theo `V1__init_schema.sql`).
...
erDiagram
    ...
    users ||--o{ bookings : "books/assigns"
    users ||--o| loyalty_accounts : "has"
    users ||--o{ customer_combos : "purchases"
    users ||--o{ notifications : "receives"
    users ||--o{ wash_sessions : "assigned_to"
    
    vehicles ||--o{ bookings : "serviced_by"
    ...
    vouchers ||--o{ voucher_tiers : "targets"
    promotions ||--o{ promotion_tiers : "targets"
    
    packages ||--o{ bookings : "selected_in"
    combos ||--o{ bookings : "selected_in"
    vouchers ||--o{ bookings : "applied_to"
    
    bookings ||--o{ booking_options : "includes"
    bookings ||--o{ booking_promotions : "applies"
    bookings ||--o| payments : "paid_via"
    bookings ||--o| wash_sessions : "scheduled_in"
    bookings ||--o{ booking_status_histories : "has_history"
    
    combos ||--o{ customer_combos : "defines"
    customer_combos ||--o{ customer_combo_usages : "redeemed_via"
    bookings ||--o| customer_combo_usages : "used_in"
    
    loyalty_accounts ||--o{ point_transactions : "transacts"
    loyalty_accounts ||--o{ tier_histories : "has"
    bookings ||--o{ point_transactions : "earns_from"
```
...
CREATE TYPE "user_account_status" AS ENUM ('PENDING', 'ACTIVE', 'BLOCKED', 'INACTIVE', 'DELETED');
CREATE TYPE "vehicle_status" AS ENUM ('ACTIVE', 'INACTIVE', 'DELETED');
...
CREATE TYPE "promotion_targeting_mode" AS ENUM ('ALL_TIERS', 'SPECIFIC_TIERS');
CREATE TYPE "booking_type" AS ENUM ('PACKAGE', 'COMBO');
CREATE TYPE "booking_status" AS ENUM ('PENDING', 'CONFIRMED', 'CHECKED_IN', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED', 'NO_SHOW');
CREATE TYPE "payment_method" AS ENUM ('CASH_AT_COUNTER', 'BANK_TRANSFER', 'E_WALLET');
CREATE TYPE "payment_status" AS ENUM ('UNPAID', 'PENDING_PAYMENT', 'PAID', 'FAILED', 'REFUNDED');
CREATE TYPE "wash_session_status" AS ENUM ('PENDING', 'QUEUED', 'CHECKED_IN', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED');
CREATE TYPE "point_transaction_type" AS ENUM ('EARN', 'REDEEM', 'EXPIRE', 'ADJUST');
CREATE TYPE "customer_combo_status" AS ENUM ('ACTIVE', 'EXPIRED', 'USED_UP', 'CANCELLED');
```

---

## 3. Các bảng chính (CREATE TABLE)
...
CREATE TABLE "users" (
  ...
  "status" varchar(20) NOT NULL DEFAULT 'PENDING' CHECK ("status" IN ('PENDING', 'ACTIVE', 'BLOCKED', 'INACTIVE', 'DELETED')),
...
### Catalog (Packages, Services & Combos)
```sql
...
### Bookings, Payments & Wash Sessions
```sql
...
## 4. Các ràng buộc Khóa ngoại (FOREIGN KEY)
