...
> **Mục đích:** Tài liệu này là nguồn tham chiếu duy nhất (single source of truth) cho AI khi thực hiện bất kỳ task frontend nào của dự án AutoWash Pro. Đọc kỹ trước khi bắt đầu code bất kỳ component, page, hoặc feature nào.
...
| **Staff Operations** | Nhân viên rửa xe | `STAFF` | `/operations/*` |
...
### 3.2 Staff Operations (`/operations/*`)

```
/operations/login          → Đăng nhập nhân viên
/operations/queue          → Hàng đợi rửa xe (bảng Kanban chính)
/operations/sessions/:id   → Chi tiết phiên rửa
/operations/check-in       → Màn hình check-in biển số
```
...
- Endpoint: `POST /bookings/:id/apply-points`
...
- Endpoint: `GET /notifications` — load danh sách
- Đánh dấu đọc: `PUT /notifications/:id/read`
...
## 19. Tech Stack Frontend (Chưa Chỉ Định — Gợi Ý)

Nếu chưa có chỉ định cụ thể, ưu tiên:
