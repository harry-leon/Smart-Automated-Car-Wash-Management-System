...
### 1.5 Ưu tiên bắt buộc phải làm trước

Đây là các yêu cầu **bắt buộc ưu tiên trước** để đáp ứng kiểm tra nền tảng hệ thống:

1. **Đăng ký tài khoản, đăng nhập, quản lý xe (CRUD)**
   - Sau khi đăng ký thành công, tài khoản khách hàng phải có hạng mặc định là `MEMBER`.

2. **Demo trọn vẹn luồng đặt lịch rửa xe**
   - Hệ thống phải quản lý được đầy đủ các trạng thái chính của booking:
     - `PENDING`
     - `CONFIRMED`
     - `CHECKED_IN`
     - `IN_PROGRESS`
     - `COMPLETED`
     - `CANCELLED`
     - `NO_SHOW` nếu đã hỗ trợ trong backend

3. **Sau khi khách check-in**
   - Hệ thống phải hiển thị phí rửa xe và/hoặc số điểm dự kiến liên quan đến booking/wash session theo dữ liệu vận hành hiện tại.

4. **Sau checkout / complete wash**
   - Điểm thưởng phải được cộng đúng.
   - Nếu đang có chương trình khuyến mãi hợp lệ, logic cộng điểm phải phản ánh được chương trình đó.
   - Chương trình khuyến mãi phải hỗ trợ:
     - áp dụng cho mọi hạng
     - hoặc áp dụng theo nhóm hạng

5. **Tăng hạng khi đủ điểm**
   - Khi khách hàng đạt ngưỡng điểm theo rule hệ thống, hạng phải được cập nhật đúng.

6. **Member phải xem được**
   - lịch sử booking
   - lịch sử rửa xe
   - lịch sử điểm / point transactions
   - các chương trình khuyến mãi của hệ thống

7. **Admin phải xem / quản lý được**
   - tất cả booking của hệ thống
   - CRUD tất cả các chương trình khuyến mãi
   - lịch sử giao dịch điểm của khách hàng

8. **Swagger / OpenAPI phải có ngay từ đầu**
   - Backend phải publish tài liệu API ngay từ giai đoạn nền tảng đầu tiên.
   - Các API ưu tiên bắt buộc phải được xem và test được qua Swagger UI hoặc OpenAPI docs.
   - Swagger là một phần của deliverable kỹ thuật, không phải việc làm sau.

Những yêu cầu trên là **mandatory-first scope**. Các phần khác như support chat, live tracking nâng cao, reminder hoàn chỉnh, report nâng cao, gateway thật chỉ nên làm sau khi nhóm yêu cầu này đã demo được end-to-end.
...
### 4.0 Ưu tiên triển khai

#### Bắt buộc làm trước

- Epic 1 — Authentication
- Epic 2 — User Profile (tối thiểu phần profile cơ bản)
- Epic 3 — Vehicle Management
- Epic 4 — Booking
- Epic 5 — Operations / Wash Session
- Epic 7 — Loyalty & Points
- Epic 8 — Promotions, Vouchers, Combos
- Epic 9 — Admin Management

#### Chỉ làm sau khi xong phần bắt buộc

- Epic 10 — Notifications, Support Chat & Live Tracking
  - ưu tiên phần notification center cơ bản nếu cần hiển thị dữ liệu
  - support chat và live tracking nâng cao không phải ưu tiên đầu

Ngoài ra, trong các Epic bắt buộc ở trên cũng cần chia nhỏ:
...
- **Must-have first**
  ...
  - loyalty earning + tier update
  - customer history views
  - admin bookings
  - admin promotions CRUD
  - admin customer point transaction history
  - Swagger / OpenAPI cho toàn bộ các API nền tảng trên

- **Can be done later**
  - voucher rule quá sâu
  - combo lifecycle nâng cao
  - report nâng cao
  - realtime polish
  - support chat
  - live tracking nâng cao
...
|---|---|---|---|
| Task 1.1 Register | `/register` | `POST /auth/register` | Validate phone/password realtime, tạo customer với hạng mặc định `MEMBER` |
...
**Yêu cầu bắt buộc cho Epic 1:**

- Các endpoint auth phải xuất hiện trên Swagger/OpenAPI ngay khi hoàn thành.
- Team frontend phải có thể dùng Swagger để kiểm tra contract thay vì đoán payload/response.
...
**Yêu cầu bắt buộc cho Epic 3:**

- Vehicle APIs phải được tài liệu hóa trên Swagger ngay khi xong để frontend test CRUD sớm.
...
Bước 3 → Chọn services     GET /services  (filter theo packageId)
...
**Booking status bắt buộc phải quản lý được trong giai đoạn ưu tiên đầu:**
...
- `COMPLETED`
- `CANCELLED`
- `NO_SHOW` nếu backend đã support trong flow vận hành

| Backend Task | Frontend Page | Endpoint | Ghi chú |
|---|---|---|---|
| Task 4.1 Packages | `/customer/bookings/new` (bước 2) | `GET /packages` | Chỉ hiện ACTIVE |
| Task 4.2 services | `/customer/bookings/new` (bước 3) | `GET /services` | Filter theo selected package |
...
**Yêu cầu bắt buộc cho Epic 4:**

- Booking APIs ưu tiên phải có Swagger để team kiểm thử create/list/detail trực tiếp trong giai đoạn nền tảng.
...
| Task 5.2 Check-in | `/staff/check-in` | `POST /operations/wash-sessions/:id/check-in` | Plate phải khớp booking, sau check-in phải hiển thị được phí/point liên quan đến wash session |
| Task 5.3 Start | `/staff/sessions/:id` | `POST /operations/wash-sessions/:id/start` | Bật countdown timer |
| Task 5.4 Complete | `/staff/sessions/:id` | `POST /operations/wash-sessions/:id/complete` | Hiện điểm loyalty earned, áp dụng rule cộng điểm theo khuyến mãi nếu hợp lệ |
...
**Yêu cầu bắt buộc cho Epic 5:**

- Operations APIs phải có Swagger để test lifecycle:
  - create session
  - check-in
  - start
  - complete
...
- Khi đủ ngưỡng điểm theo rule hệ thống, khách hàng phải được tăng hạng đúng.
...
Các tab `Booking`, `Lịch sử rửa`, `Giao dịch điểm` là **ưu tiên bắt buộc phải có trước** để đáp ứng yêu cầu demo nền tảng quản lý và history.
...
- ✅ Publish OpenAPI / Swagger documentation ngay từ đầu
